...
public class LearningTools {
    ...
    public QuizResult generateQuiz(
        ...
        try {
            List<Question> qs = llm.chatJson(system, user, new TypeReference<List<Question>>() {});
            if (qs == null || qs.isEmpty()) return fallbackQuiz(count);
            return new QuizResult(qs.subList(0, Math.min(count, qs.size())), "PRACTICE");
        } catch (Exception e) {
    ...
    public TutorResult answerQuestion(
        ...
        try {
            ...
            if (r.success()) {
                return new TutorResult(r.content(), false);
            }
        } catch (Exception e) {
     ...
     */
    @Tool(description = """
            根据用户的做题表现(总题数 / 正确数)生成复盘反馈。
            返回:summary 总结、strengths 优点、weaknesses 不足、nextSteps 下一步建议。
            在以下情况调用:
              - 用户提交答案后
              - 用户说"复盘"、"总结"、"看看我学得怎么样"等
            """)
    public ReviewResult reviewProgress(
            @ToolParam(description = "总题数") int total,
            @ToolParam(description = "正确题数") int correct) {
        log.info("Tool.reviewProgress: total={}, correct={}", total, correct);
        double accuracy = total == 0 ? 0 : (correct * 1.0 / total);

        String system = """
                你是学习复盘教练。根据做题表现生成简短反馈。
                严格 JSON:{"summary":"...","strengths":["..."],"weaknesses":["..."],"nextSteps":["..."]}""";
        String user = String.format("总题数:%d,正确:%d,正确率:%.0f%%", total, correct, accuracy * 100);

        try {
            ReviewResult result = llm.chatJson(system, user, ReviewResult.class);
            if (result != null) return result;
        } catch (Exception e) {
            log.warn("reviewProgress failed: {}", e.getMessage());
        }
        return new ReviewResult(
                "完成 " + total + " 题,正确 " + correct + " 题",
                List.of("基础概念掌握尚可"),
                List.of("细节应用需加强"),
    ...
    private QuizResult fallbackQuiz(int count) {
    ...
    public record QuizResult(List<Question> questions, String quizType) {}

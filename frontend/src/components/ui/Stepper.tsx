...
/**
 * Stepper 步进器 — 来自 react-bits
 * https://reactbits.dev/components/stepper
 *
 * 本项目用于 Agent 调用链的可视化:每一步展示一个 Agent 调用的详情。
 * 配色做了 brand 主题适配:
 *   - 完成色:#10b981 (emerald-500)
 *   - 活跃色:#4f46e5 (brand-600)
 *   - 圆角:rounded-3xl (v3 兼容)
 */
export default function Stepper({
  ...
  return (
    <div className="flex min-h-full flex-1 flex-col items-center justify-center p-2" {...rest}>
      <div
        className={`mx-auto w-full max-w-md rounded-3xl shadow-lg border border-slate-200 bg-white ${stepCircleContainerClassName}`}
      >
          ...
          <div className={`px-6 pb-6 ${footerClassName}`}>
            <div className={`mt-6 flex ${currentStep !== 1 ? "justify-between" : "justify-end"}`}>
              {currentStep !== 1 && (
                <button
                  onClick={handleBack}
                  className={`duration-350 rounded px-3 py-1.5 text-sm transition ${
                    currentStep === 1
                      ? "pointer-events-none opacity-50 text-slate-300"
              ...
              <button
                ...
                className="duration-350 flex items-center justify-center rounded-full bg-brand-600 py-1.5 px-4 text-sm font-medium tracking-tight text-white transition hover:bg-brand-700 active:bg-brand-800"
                {...nextButtonProps}
              >
    ...
    >
      <motion.div
        variants={{
          ...
          complete: { scale: 1, backgroundColor: "#4f46e5", color: "#4f46e5" },
        }}
